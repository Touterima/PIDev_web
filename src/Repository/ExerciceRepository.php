...
class ExerciceRepository extends ServiceEntityRepository
{
    ...
    public function getExerciseDistributionByCategory()
    {
        $query = $this->createQueryBuilder('e')
            ->select('c.nom as category', 'COUNT(e.exerciceId) as exercise_count') // Get category name and count exercises
            ->join('e.categorie', 'c') // Join with category
            ->groupBy('c.categorieId') // Group by category
            ->getQuery();

        return $query->getResult(); // Returns the category names with the count of exercises
    }
//    /**
